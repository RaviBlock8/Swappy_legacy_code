...
import { getTimeLimit } from "./getTimeLimit";
...
const getAmountOut = async (
	...
	try {
		...
		denominator = JSBI.add(
			denominator,
			JSBI.multiply(inputAmount.bigInt, JSBI.BigInt(997))
		);
		return JSBI.divide(numerator, denominator).toString();
	} catch (error) {
		console.log("Error: ", error);
...
const swapExactTokensForTokens = async (
	...
	try {
		...
		let allowance = await inputAmount.token.functions.allowance(
			account,
			router.address
		);
		...
		minOutputAmount.numerator = JSBI.multiply(
			minOutputAmount.numerator,
			JSBI.BigInt(minimumPercentage)
		);
		minOutputAmount.numerator = JSBI.divide(
			minOutputAmount.numerator,
			JSBI.BigInt(100)
		);
